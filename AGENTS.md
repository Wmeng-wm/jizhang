...
### 2026-08-19：导出账单以导出日期为标题（已部署，v32）

- **需求**：导出的账单也按「导出的当天日期」做标题（对齐归档的做法）
- **`exportAccount`**：文件名 `工作账单_2026-08-19.csv`（个人/全部同理）；CSV 首行
  `工作账户财务报表（导出日期 2026年8月19日 周三）`；概要区新增 `导出日期,,2026-08-19` 行
- **`exportStatsResult`**：标题加导出日期，概要区新增 `导出日期` 行（保留「日期范围」行）
- **`exportReimbursementCSV`**：标题 `报销单（导出日期 …）`（保留「生成日期」行）
- **顺手修 bug**：`exportAccount` 的 CSV 首字符原是 `'\\ufeff'`（字面文本，非 BOM）→ 改为真 BOM `'\ufeff'`
- **新函数**：`fmtCnDate(stamp, withWeekday)`（2026-08-19 → 2026年8月19日 [周三]）供导出/归档共用，
  `fmtArchiveStamp` 内部改为调用它；版本标记 v31 → v32
